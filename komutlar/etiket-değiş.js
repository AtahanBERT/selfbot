...
try {
 client.user.setDiscriminator(args[0], client.password || process.env.password).then(() => {
...
} catch {message.reply({content:`${basarisiz} ${message.author}, Etiketini çok hızlı değişiyosun 1 saat sonra tekrar dene.`}).then(x => setTimeout(() => {x.delete()}, 5000))
}
}
};
